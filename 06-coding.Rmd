...
Here, we introduce repeated difference, Helmert, and polynomial contrasts.^[If one is trying to set a general prior for differences between means, then the function `bayestestR::contr.equalprior()` can be used, which sets equal marginal priors on all means and differences between them.] 
...
Normally, one may expect the pattern to be monotonically decreasing, i.e., higher word frequency should lead to faster reading times. However, we choose this pattern because we think it may be helpful in understanding how polynomial and monotonic contrasts work (see below).
...
```{r, echo=TRUE}
Xpol2           <- data.frame(linear=c(F1=-3,F2=-1,F3= 1,F4= 3)/2)
