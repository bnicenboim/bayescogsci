...
A popular way to evaluate and compare models is to investigate their ability to make predictions for "out-of-sample data," that is, to use what we learned from the observed data to predict future or \index{Unseen observation} unseen observations. \index{Cross-validation} Cross-validation is used to test which of the models under consideration is/are able to learn the most from our data in order to make better predictions. In cognitive science, the goal is only rarely to predict future observations; more often, the researcher wants to compare the performance of different models given some observed data. Nevertheless, there are situations in cognitive science where evaluating the predictive performance of models becomes important.
...
In order to treat the entire data set as a held-out set and to evaluate the predictive accuracy \index{Predictive accuracy} using every observation, one successively changes what constitutes the training set and the held-out set by refitting the model and evaluating it in different folds. This ensures that the predictions of the model are tested over the entire data set.
...
\begin{equation}
p(y_{pred}\mid y )=\int_\Theta p(y_{pred}\mid \Theta) p(\Theta\mid y)\, \mathrm{d}\Theta
(\#eq:postpp3)
\end{equation}
...
\begin{equation}
p(y_{pred}\mid y,  \mathcal{M}_1)=\int_\Theta p(y_{pred}\mid \Theta, \mathcal{M}_1) p(\Theta\mid y, \mathcal{M}_1)\, \mathrm{d}\Theta
(\#eq:postpp2)
\end{equation}
...
\begin{equation}
elpd = u(\mathcal{M}_1) = \int_{y_{pred}} p_t(y_{pred}) \log p(y_{pred} \mid y, \mathcal{M}_1) \, \mathrm{d} y_{pred}
(\#eq:elpd)
\end{equation}
...
This quantity indicates the predictive accuracy of the model $\mathcal{M}_1$ for a single observation, and it is reported in the package \index{\texttt{loo}} `loo`, which also \index{brms} `brms` relies on. In addition, the `loo` package uses the sum of the expected log pointwise predictive density, $\sum \widehat{elpd}_n$ (Equation \@ref(eq:approxelpd) without $\frac{1}{N}$) as a measure of predictive accuracy (this is referred to as \index{\texttt{elpd\_loo}} `elpd_loo` or \index{\texttt{elpd\_kfold}} `elpd_kfold` in the packages `loo` and `brms`). For model comparison, the difference between the $\sum \widehat{elpd}_n$  of competing models can be computed, including the standard deviation of the sampling distribution of the difference. It's important to notice that we are calculating an approximation to the expectation $elpd$ that we actually want to compute, and thus we always need to consider its inherent randomness (due to having a limited amount of data), which is quantified by the standard error [@vehtariLimitationsLimitationsBayesian2019].
...
```{r diffpredacc, fig.cap = "The difference in predictive accuracy between a model including the effect of cloze and a null model. A larger (more positive) difference indicates an advantage for the model that includes the effect of cloze.", fig.height=2.2 }
...
The summary shows that $\hat{k}$ estimates are ok.
...
The estimated $elpd$ based on PSIS-LOO-CV has several large $\hat{k}$ values.
...
Here, cross-validation shows a clear advantage for the model with the log-normal likelihood. Figure \@ref(fig:diffpredacclog12)(a) visualizes the pointwise predictive accuracy.

```{r diffpredacclog, fig.keep ="none"}
df_stroop <- df_stroop %>%
...
Figure \@ref(fig:diffpredacclog12)(a) shows that at first glance, the advantage of the log-normal likelihood seems to lie in being able to capture extremely slow observations. Figure \@ref(fig:diffpredacclog12)(b) zooms in to visualize the pointwise predictive accuracy for observations with response times smaller than two seconds.

```{r diffpredacclog2, fig.keep ="none"}
ggplot(df_stroop, aes(x = RT, y = diff_elpd)) +
  geom_point(alpha = .3) +
  xlab("RC (ms)") +
...
(ref:diffpredacclog12) (a) Difference in predictive accuracy between Stroop models with log-normal and normal likelihoods, where a more positive value favors the log-normal model. (b) Difference in predictive accuracy between the same models, focusing on observations under two seconds, with positive values again favoring the log-normal model.

```{r diffpredacclog12, message = FALSE, echo = FALSE, fig.cap = "(ref:diffpredacclog12)", fig.show = "hold", out.width="49%", fig.width = 3.9, fig.height =3.2 }

ggplot(df_stroop, aes(x = RT, y = diff_elpd)) +
  geom_point(alpha = .4) +
  xlab("RC (ms)") +
  ggtitle("(a) Whole range of observations")
...
ggplot(df_stroop, aes(x = RT, y = diff_elpd)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  coord_cartesian(xlim = c(0, 2000), ylim = c(-10, 10)) +
  ggtitle("(b) Observations smaller than 2 seconds")
```

Figure \@ref(fig:diffpredacclog12)(b) suggests that the advantage of the log-normal likelihood seems to lie in being able to account for most of the observations in the data set, which occur around the $500$ ms.
