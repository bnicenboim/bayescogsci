...
```{r diffpredacc, fig.cap = "The difference in predictive accuracy between a model including the effect of cloze and a null model. A larger (more positive) difference indicates an advantage for the model that includes the effect of cloze.", fig.width=2.2 }
...
ggplot(df_stroop, aes(x = RT, y = diff_elpd)) +
  geom_point(alpha = .4) + xlab("RC (ms)") +
  ggtitle("(a) Predictive accuracy for the whole range of observations")
...
ggplot(df_stroop, aes(x = RT, y = diff_elpd)) +
  ...
  geom_hline(yintercept = 0, linetype = "dashed") +
  coord_cartesian(xlim = c(0, 2000), ylim = c(-10, 10)) +
  ggtitle("(b) Predictive accuracy for observations smaller than 2 seconds")
```
