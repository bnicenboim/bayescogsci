...
One way to investigate how variable the outcome of Bayes factor analyses can be (given that the Bayes factor is computed in a stable and accurate way), is to generate some realistic data and compute Bayes factors for these simulated data. A key question then is how to set priors that yield realistic simulated data sets. Here, we use the posteriors from the analysis of a real empirical data set and generate posterior predictive distributions, which serve as our realistic simulated data.  Computing the Bayes factor analysis again on the simulated data can provide some insight into how variable the Bayes factor will be.

Here, we perform these simulations. We start with a data set and a model [the Bayesian hierarchical model fitted to the data from @lago2015agreement], and use the posterior distributions from this model to generate posterior predictive data. In these simulations, one takes posterior samples for the model parameters (i.e., $p(\boldsymbol{\Theta} \mid \boldsymbol{y})$), and for each posterior sample of the model parameters, one can simulate new data $\tilde{\boldsymbol{y}}$ from the model $p(\tilde{\boldsymbol{y}} \mid \boldsymbol{\Theta})$.
...
The question that we are interested in here now is, how much information is contained in this simulated data. That is, we can run Bayesian models on these simulated data sets and compute Bayes factors to test whether in the simulated data there is evidence for agreement attraction effects.
...
We can now visualize the distribution of Bayes factors ($BF_{10}$) across the simulated data by plotting a histogram.
...
fix.tmp <- posterior_samples(m1_lagoE1, "b_x") %>% summarize(Estimate=mean(b_x), Q2.5=quantile(b_x,0.025), Q97.5=quantile(b_x,0.975)) %>% as.numeric()

datTp <- datT[, c("Study", "Estimate", "Q2.5", "Q97.5")]
#datTp <- rbind(c(0, fixef(m1_lagoE1)[2, c("Estimate", "Q2.5", "Q97.5")]), datTp)
datTp <- rbind(c(0, fix.tmp), datTp)
...
The results show that the Bayes factors are quite variable. The Bayes factor results differ in that they either provide strong evidence for the alternative model ($BF_{10} > 10$) or moderate evidence for the null model ($BF_{10} < 1/3$), despite the fact that all data sets are simulated from the same distribution. The majority of the simulated data sets support the alternative model with moderate to weak evidence. In other words, this analysis reveals the \index{Dance of the Bayes factors} "dance of the Bayes factors" with simulated repetitions of the same study, similar to the "dance of $p$-values" [@cumming2014new].^[See http://daniellakens.blogspot.com/2016/07/dance-of-bayes-factors.html.] The variation in these findings demonstrates a very important point that is not widely appreciated: the evidence we get from a particular Bayes factors calculation may not hold up if the same study is replicated. Just obtaining a large Bayes factor alone is not necessarily informative; the variation in the Bayes factor under (hypothetical or actual) repeated sampling needs to be considered as well. For a real-life example, see the replication attempt reported in @Schoknecht2025.
...
3. Then do the following for increasing sample sizes $n$:
    ...
    2. Fit the model to the simulated data using regularizing (or principled) priors [these are called fitting priors in @wang2002simulation], and derive the posterior distribution each time, and compute the Bayes factor using a null model that assumes a zero effect for the parameter of interest.
