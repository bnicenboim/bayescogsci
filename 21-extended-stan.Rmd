...
Stan's sampler can explore the parameter space more easily if its step size is appropriate for all the parameters. This is achieved when there are no strong  dependencies between parameters. We want to assume the following.
...
Implementing the simulation-based calibration algorithm "by hand" introduces a new source of potential errors as we did in section \@ref(sec-validSBC). Fortunately, the R package  \index{SBC} `SBC` [@R-SBC] provides tools to validate a Stan model (or any sampling algorithm) by allowing us to run simulation-based calibrations easily.
