...
* The *Marginal Likelihood*, $p(\boldsymbol{y})$, was introduced in chapter \@ref(ch-intro) and standardizes the posterior distribution to ensure that the area under the curve of the distribution sums to 1, that is, it ensures that the posterior is a valid probability distribution.
...
The expression on the right-hand side corresponds to a beta distribution with parameters $a=84$, and $b=24$. This becomes evident if we rewrite the right-hand side such that it represents the core part of a beta PDF (see equation \@ref(eq:betach2)). All that is missing is a normalizing constant  which would make the area under the curve sum to one.
...
```{r postbeta-viz,echo=FALSE,fig.cap = "(ref:postbeta-viz)"}
## Likelihood
k <- 80
n <- 100
## Prior
...
The \index{Gamma distribution} Gamma distribution is defined in terms of the parameters $a$, $b$: $\mathit{Gamma}(a,b)$. If there is a random variable $Y$ (where $y\geq 0$) that has a Gamma distribution as a PDF ($Y\sim \mathit{Gamma}(a,b)$), then:

\begin{equation}
\mathit{Gamma}(y | a,b)=\frac{b^a y^{a-1} \exp(-by)}{\Gamma(a)}
\end{equation}

Suppose that we have $n$ data points,  $x_1,\dots, x_n$, that are drawn from an  exponentially distributed. The \index{Exponential likelihood} exponential likelihood function is:

\begin{equation}
p(x_1,\dots,x_n | \lambda)=\lambda^n \exp (-\lambda \sum_{i=1}^n x_i )
\end{equation}

It turns out that if we assume a $\mathit{Gamma}(a,b)$ prior distribution for $\lambda$ and the above exponential likelihood, the posterior distribution of $\lambda$ is a Gamma distribution. In other words, the $\mathit{Gamma}(a,b)$ prior on the $\lambda$ parameter in the exponential distribution will be written:
...
- Given equation \@ref(eq:weightingpoisga), show that as $n$ increases (as sample size goes up), the maximum likelihood estimate $\bar{x}$ dominates in determining the posterior mean, and when $n$ gets smaller and smaller, the prior mean dominates in determining the posterior mean.
