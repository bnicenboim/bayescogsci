...
As noted previously, generating prior predictive distributions can be computationally slow if done naively in R with a for-loop; for instance, producing $1000$ samples of the prior predictive distribution for our model from section \@ref(sec-priorpred) results in $361000$ predicted values, which takes a few seconds to compute. Although this approach works, it is not optimal for larger models or datasets. 
...
The following function is an edited version of the earlier  `normal_predictive_distribution` from the online section \@ref(sec-efficientpriorpd) in section \@ref(sec-priorpred); it has been edited to make it compatible with logistic regression and dependent on set size.
...
## Some important (and sometimes confusing) points: {#app-hierarchical}
...
## Treatment contrast with \index{Intercept} intercept as the \index{Grand mean} grand mean. {#app-cTreatGM}
