...
Informally, we all understand what the term \index{Probability} *probability* means. We routinely talk about things like the probability of it raining today. However, there are two distinct ways to think about probability. One can think of the probability of an event with reference to the \index{Frequency} frequency with which it might occur in repeated observations. Such a conception of probability is easy to imagine in cases where an event can, at least in principle, occur repeatedly. An example would be obtaining a 6 when tossing a die again and again. However, this frequentist view of probability is difficult to justify when talking about certain one-of-a-kind events, such as earthquakes. In such situations, probability is expressing our uncertainty about the event happening.
...
* Two events are independent if and only if the probability of both happening is equal to the product of the probabilities of each event happening.
...
The actually observed outcome (a 0,1 response; reading time, etc.) will be distinguished from the random variable that generated it by using lower case $y$. We can call $y$ an instance of $Y$; every new observed outcome will be slightly different due to random variability.
...
So, stating the definition of random variables formally [following @blitzstein2014introduction], we define a random variable $Y$ as a function from a sample space of possible \index{outcome} outcomes $S$ to the real number system:^[The actual formal definition of random variable is more complex, and it is based on measure theory. A more rigorous definition can be found in, for example, @steyer2017probability and @resnick2019probability.]

\begin{equation}
Y : S \rightarrow \mathbb{R}
\end{equation}

The random variable associates to each outcome $\omega \in S$ exactly one number $Y(\omega) = y$. Suppose that $y$ represents all the possible values that the random  variable generates; these values are taken to belong to the support of the random variable $Y$: $y \in S_Y$.

As a concrete example, consider an experiment where we ask subjects to respond to 10 questions that can either have a correct or incorrect answer. A possible outcome is the following sequence of correct (represented as c) and incorrect (represented as i) answers: $ciciccicci$. Another possible outcome is $cciicicici$. The random variable would associate a number with each possible outcome; for example, the number of correct responses. So, for outcome $ciciccicci$ the random variable could be defined to return the number 6 (the number of correct responses); and for $cciicicici$ the number 5.  The outcomes $\omega$ are the sequences of correct and incorrect answers: $S=\{cciicicici,ciciccicci,...\}$. And $y \in S_Y$ can have the values $0$ or $1$,..., $10$; thus, $S_Y$ is a subset of $\mathbb{R}$.

An \index{event} event is distinguished from an outcome as follows: an event is a subset of the set of outcomes. For example, in a particular experiment with a particular subject, the actually observed event would be a set with only one element, an outcome, for example cciicicici. We will say that the event $\{cciicicici\}$ occurred.

We will say that the number of correct responses from a subject is generated from a random variable $Y$. Because only discrete responses are possible (the number of correct responses can be $0, 1, 2, \ldots, 10$), this is an example of a *discrete random variable*.

This random variable will be assumed to have a parameter $\theta$ that represents the probability of producing a correct response (as discussed below, you will see that the random variable in question is the binomial random variable). Given some observed data that is assumed to come from a particular distribution, typically our goal is to obtain an estimate of the (unknown) value of the parameter associated with that distribution. More generally, if there is more than one parameter involved in the distribution, then our goal is to obtain an estimate of these parameters.

Now, suppose that in our above example, the random variable $Y$ gives us one correct answer; this will be written as $Y=1$. The  outcomes that could produce $1$ are any of this set of ten possible outcomes: $\{ciiiiiiiii,iciiiiiiii,iiciiiiiii,iiiciiiiii,...\}$.
The set $\{ciiiiiiiii,iciiiiiiii,iiciiiiiii,iiiciiiiii,...\}$ is a subset of the sample space, so it is an event.
The number $y=1$ thus represents this event (set of outcomes), and will have a probability of occurring associated with it; this is defined next.
...
The quantity computed for the values $1$ and $2$ are *not* the probability of observing $1$ or $2$ in this distribution. As mentioned earlier, probability in a continuous distribution is defined as the area under the curve, and this area will always be zero at any point value (because there are infinitely many different possible values). If we want to know the probability of obtaining values between an upper and lower bound $b$ and $a$, i.e., $P(a<Y<b)$ where these are two distinct values, we must use the \index{Cumulative distribution function} cumulative distribution function or \index{CDF} CDF: in R, for the normal distribution, this is the `pnorm()` function. For example, the probability of observing a value between $+2$ and $-2$ in a normal distribution with mean $0$ and standard deviation $1$ is:
...
<!--chapter:end:01-introductionBayesCogSci.Rmd-->
