...
```{r LDT-tikz,engine='tikz',fig.ext= if (knitr::is_html_output()) "svg" else "pdf", echo = FALSE, fig.cap = "(ref:LDT-tikz)"}
...
```{r rtlexical, fig.cap = "(ref:rtlexical)", message = FALSE, warning= FALSE, fold = TRUE}
...
```{r qppfreq, fig.cap = "(ref:qppfreq)", warning = FALSE}
...
```{r lognormalrace, echo = FALSE, fig.cap = "A schematic illustration of the log-normal race model for the lexical-decision task with a word stimulus. A larger rate of accumulation (V) leads to a larger slope. Here, the choice of word is selected."}
set.seed(100)
D_w <- 10000
D_nw <- 10000
mu_v_w <- log(20)
...
ggplot(data_acc, aes(xend = T)) +
  ...
  annotate(geom="segment", x = -350, xend = 0, y= 300,yend=300,arrow = arrow(length = unit(0.5,"cm")))+
  annotate(geom="text", x =-150, y=1200,label = "non-decision\ntime")+

  annotate(geom="segment", x = 1400, xend = 1400, y= 0,yend=D_w,arrow = arrow(length = unit(0.5,"cm")))+
  annotate(geom="text", x =1600 , y=5800,label = "Decision\nthreshold (D)")+
  geom_segment(data = onetrial, aes(x = 0, xend = T, y= 300,yend=300),arrow = arrow(length = unit(0.5,"cm")))+
  annotate(geom="text", x =320, y=750,label = "Decision time (T)")

```

A log-normal distribution is partly justified by the work by @ulrichInformationProcessingModels1993 (also see Box \@ref(thm:lognormal)), and as discussed later, it is very convenient mathematically.
...
```{r scatterracesim, fig.cap = "(ref:scatterracesim)", message = FALSE, warning= FALSE, fold = TRUE}
acc_lbl <- as_labeller(c(`0` = "Incorrect", `1` = "Correct"))
...
Following @RouderEtAl2015, we assume that the \index{Noise parameter} noise parameter is the same for each accumulator, since this means that contrasts between \index{Finishing time distribution} finishing time distributions are captured completely by contrasts of the locations of the log-normal distributions. We discuss at the end of the chapter why one would need to relax this assumption (also see Exercise \@ref(exr:lnracescale)).
...
```{r thetac, fig.cap = "(ref:thetac)", fold = TRUE}
...
```{r tnd, fig.cap = "(ref:tnd)", fold = TRUE}
sdlog <- .6
...
```{r qppfreqpp, fig.cap = "(ref:qppfreqpp)", warning = FALSE}
