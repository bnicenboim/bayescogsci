...
If we modify the meta-analysis `brms` model in section \@ref(sec-brmsmeta) by setting `sigma = TRUE` in `resp_se()` function, we won't be able to get estimates for $\zeta_{n}$. This is because these estimates will be handled implicitly. The model presented formally in Equation \@ref(eq:ma0), repeated here as \@ref(eq:ma0again) is equivalent to the one in Equation \@ref(eq:ma2).  A critical difference is that $\zeta_n$ does not appear any more.
