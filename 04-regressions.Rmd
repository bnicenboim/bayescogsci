...
```{theorem, truncation}
**\index{Truncated distribution} Truncated distributions**
```
...
```{theorem, intercept}
**\index{Intercept} Intercepts in `brms`**
```
...
```{r centered-non-centered, echo = FALSE, fig.cap ="(ref:centered-non-centered)", fig.pos = "H", out.extra = ''}
...
```{r postpredmean, fig.cap ="(ref:postpredmean)", message= FALSE, fig.height = 2, fig.show = "hold", fig.align='center', fig.pos = "H", out.extra = ''}
for (l in 0:4) {
  df_sub_pupil <- filter(df_pupil, load == l)
  p <- pp_check(fit_pupil,
                type = "stat",
...
```{theorem, lognormal}
**Understanding the \index{Log-normal likelihood} log-normal likelihood**
```
...
```{r logexp, fig.cap="(ref:logexp)", echo = FALSE, fig.pos = "H", out.extra = '', fig.height = 2}
df_a <- tibble(
...
```{r expgd, fig.cap="(ref:expgd)", echo = FALSE, fig.pos = "H", out.extra = ''}
N <- 300
df_dg <- tibble(
  n = rep(seq(1, N, 5), 2),
  Intercept = 6,
...
**\index{Log-normal distribution} Log-normal distributions everywhere**
...
```{theorem, priorR}
**\index{Prior predictive check} Prior predictive checks in R**
```
