...
Keep in mind that different textbooks have slightly different ways of presenting the underlying structure of what constitutes a \index{Probability space} probability space (defined below).^[Our presentation is based on lecture notes from the MSc Statistics program in Sheffield, UK.]
...
An \index{Event} event is distinguished from an outcome as follows: an event is a subset of the set of outcomes. For example, in a particular experiment with a particular subject, the event corresponding to obtaining one correct answer when 10 questions are asked, is the subset of outcomes {ciiiiiiiii,iciiiiiiii,iiciiiiiii,...}; in other words, the event corresponding to obtaining nine correct answers is a set containing 10 elements. Similarly, the event corresponding to obtaining nine correct answers when 10 questions are asked is the subset of outcomes {ccccccccci,ccccccccic,...}.
When we conduct an experiment, if we get a particular outcome like ciiiiiiiii, then we say that the event  {ciiiiiiiii,iciiiiiiii,iiciiiiiii,...} occurred.
To characterize all possible events mathematically, we can define a set $F$ that contains all possible subsets of the sample space. Notice that member of the set $F$ includes each of the possible outcomes as singleton sets (sets with one element): {ciciccicci}, etc. These can be called \index{Elementary events} elementary events.
