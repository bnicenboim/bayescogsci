...
If we want to use `brms` to generate prior predictive data in this manner even before we have any data, we do need to include a data frame  (at least for the current version of `brms`). Setting `sample_prior = "only"` will ignore the data frame.
