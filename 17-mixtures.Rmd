...
To simplify matters, we ignore the possibility that the accuracy of the choice is also affected by the difficulty of the choice. Also, we ignore the possibility that subjects might be biased to one specific response in the guessing mode, but see online exercise \@ref(exr:mixbias).
...
Once we change the upper bound of `gamma` in the `parameters` block, we also need to truncate the distribution in the `model` block by correcting the PDF with its CDF. This correction is carried out using the CDF because we are truncating the distribution at the right-hand side; recall that earlier we used the complement of the CDF when we truncate a distribution at the left-hand side); see online section \@ref(app-truncation).
...
```{r traceplotmulti, fig.cap = "(ref:traceplotmulti)", message = FALSE, fig.height = 3.5 }
...
```{r traceplotnomulti, fold=TRUE, fig.cap = "(ref:traceplotnomulti)", message = FALSE , fig.height = 3.5}
...
First, take a look at the general distribution of response times generated by the posterior predictive model and by our real data in Figure \@ref(fig:postpmix-postpmanip)(a).

```{r postpmix, message = FALSE, fig.keep = "none"}
...
Next, examine the effect of the experimental manipulation in Figure \@ref(fig:postpmix-postpmanip)(b): The posterior predictive check reveals that the model underestimates the observed effect of the experimental manipulation: the observed difference between response times is well outside the bulk of the predictive distribution.

```{r postpmanip, message = FALSE, fig.keep = "none"}
meanrt_diff <- function(rt){
...
(ref:postpmix-postpmanip) (a) Posterior predictive distribution of the hierarchical fast-guess model (`mixture_h2_gen.stan`) compared to observed response times. (b) Posterior predictive distribution of the response time difference due to experimental manipulation, using the same model; the vertical bar indicates the observed difference in the data.

```{r postpmix-postpmanip, fig.cap = "(ref:postpmix-postpmanip)", message = FALSE, echo=FALSE, fig.show = "hold", out.width="49%", fig.width = 3.9, fig.height =3.2}
# a
ppc_dens_overlay(y = ls_dots_data_short$rt, yrep = rt_pred[1:100,]) +
  coord_cartesian(xlim = c(0, 2000)) +
  ggtitle("(a) Posterior predictive distribution\n")

# b
ppc_stat(ls_dots_data_short$rt,
         yrep = rt_pred,
         stat = meanrt_diff) +
  ggtitle("(b) Posterior predictive distribution\nof differences in RTs")
```
