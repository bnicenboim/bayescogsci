...
```{r traceploteeg2, fig.cap ="(ref:traceploteeg2)"}
...
```{r scatterpairs, fig.cap = "(ref:scatterpairs)"}
...
```{theorem, noncenterparam}
**A simple non-centered parameterization** 
```
...
```{r traceploteeg3, fig.cap ="(ref:traceploteeg3)"}
...
```{r pairstauu, fig.cap ="(ref:pairstauu)", fold = TRUE}
...
```{r pairstauz, fig.cap ="(ref:pairstauz)", fold = TRUE}
mcmc_pairs(as.array(fit_eeg3),
...
```{r traceploteegcorr, fig.cap ="(ref:traceploteegcorr)", fold = TRUE}
...
The solution to this problem is the reparameterization of this model. The reparameterization for this type of model  requires us to use \index{Cholesky factorization} Cholesky factorization [@fieller]. The mathematics and the intuition behind this parameterization are explained in Box \@ref(thm:cholesky).
...
```{r posteegcorr2, fig.height=2, message=FALSE, fig.cap ="(ref:posteegcorr2)"}
